(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
            ...
            angular) {
            return (root.ngRndr.plugins.pivotData.renderers.pivotDataUI = factory($,
                angular));
        });
    } else if (typeof module === 'object' && module.exports) {
        module.exports = (root.ngRndr.plugins.pivotData.renderers.pivotDataUI = factory(require('jquery'),
            require('angular')));
    } else {
        root.ngRndr.plugins.pivotData.renderers.pivotDataUI = factory(root.$,
            root.angular);
    }
}(this, function($,
    angular) {
    return function(renderingEngine, opts) {
        ...
        var pivotDataUIConfig = function($mdThemingProvider, $ngRndrRenderersProvider, $ngRndrDataViewsProvider, $ngRndrAggregatorsProvider) {
            //Define app palettes
            ...
            $.each(opts.dataViews.list(), function(index, dataViewName) {
                $ngRndrDataViewsProvider.add(dataViewName,
            ...
            $.each(opts.renderers.list(), function(index, rendererName) {
                if ((rendererName !== renderingEngine.renderer) &&
                    ...
                    $ngRndrRenderersProvider.add(rendererName,
            ...
            $.each(opts.aggregators.list(), function(index, aggregatorName) {
                $ngRndrAggregatorsProvider.add(aggregatorName,
                    opts.aggregators[aggregatorName].aggregate
                );
            });
        };

        pivotDataUIConfig.$inject = ['$mdThemingProvider', '$ngRndrRenderersProvider', '$ngRndrDataViewsProvider', '$ngRndrAggregatorsProvider'];
        ...
        var renderingEngineFactory = new injector.get('$ngRndrRenderingEngine');
        ...
        var embeddedRenderingEngine = new renderingEngineFactory(renderingEngine.dataView.meta.renderer, renderingEngine.id, renderingEngine.aggregator.name, renderingEngine.aggregator.aggInputAttributeName, renderingEngine.dataView.meta, renderingEngine.derivedAttributes, renderingEngine.locale, renderingEngine.sorters, null, renderingEngine.dataView.data);
